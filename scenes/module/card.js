...
export const crearCard = (scene, status, name, age, description, namePhoto, namePlataforma,contenedor) => {
  ...
  contenedor.setDepth(1); // Ajusta el valor según sea necesario, un valor mayor lo colocará más arriba
      ...
      let colision = scene.physics.add.collider(
      ...
      scene.input.keyboard.on('keydown', (event) => {
        ...
        if (event.key === 'ArrowUp' || event.key === 'ArrowDown' || event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
          // El avatar está en movimiento
  
         
           console.log("El avatar está en movimiento.");
        contenedor.setAlpha(0);
        }
      });
        return;
